...
class ChannelFetcher:
    """Fetches posts from Telegram channels and other dialog types."""
    ...
    def __init__(
        ...
        # Initialize dialog types to fetch: 'channel', 'chat', 'megagroup', 'supergroup', 'broadcast'
        self.dialog_types = dialog_types or ["channel"]
    ...
    async def fetch_all_channels(self) -> List[Dict]:
        ...
        if "all" in self.dialog_types:
            # Fetch from all dialogs
            dialogs = await self.client.get_dialogs()
            filtered_dialogs = [
                d for d in dialogs if d.is_channel or d.is_group or d.is_user
            ]
            logger.info(f"Found {len(filtered_dialogs)} dialogs to fetch from")

            for dialog in filtered_dialogs:
                # Filter by dialog type
                if self.dialog_types and not self._dialog_type_matches(dialog):
                    continue

                entity = dialog.entity
                name = getattr(entity, "username", None) or getattr(entity, "title", None) or str(entity.id)
                channel_posts = await self._fetch_channel_posts(name)
            ...
            for channel in channels:
                if self.dialog_types and "channel" not in self.dialog_types:
                    continue
    ...
    async def fetch_and_save(self) -> bool:
            ...
            for dialog in filtered_dialogs:
                ...
                try:
                    ...
                    if channel_posts:
                        ...
                        if not success:
                            logger.error(f"Failed to save posts for {name}")
                            error_count += 1
                        else:
                            logger.info(f"Successfully saved {len(channel_posts)} new posts from {name}")
                except Exception as e:
        ...
        logger.info(f"Fetch and save complete. Added {new_post_count} new posts")
        if error_count > 0:
            logger.warning(f"Encountered errors with {error_count} dialogs/channels")
    ...
    def _dialog_type_matches(self, dialog) -> bool:
        """Check if dialog matches the requested dialog types."""
        if not self.dialog_types:
            return True

        # Map dialog types to Telethon dialog properties
        dialog_type_map = {
            "channel": lambda d: d.is_channel and not d.is_group,
            "chat": lambda d: d.is_user,
            "group": lambda d: d.is_group,
            "megagroup": lambda d: getattr(d.entity, "megagroup", False),
            "supergroup": lambda d: d.is_group and not getattr(d.entity, "megagroup", False),
            "broadcast": lambda d: d.is_channel and getattr(d.entity, "broadcast", False),
        }

        for dtype in self.dialog_types:
            check_func = dialog_type_map.get(dtype)
            if check_func and check_func(dialog):
                return True
